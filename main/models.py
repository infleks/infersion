...
class ProductVersion(models.Model):
    ...
    def getModelsByTotalOrder():
        orderedVersions = []
        prods = ProductInfo.objects.all().order_by("productName")
        for p in prods:
            mdls = ProductModule.objects.filter(
                product=p).order_by("moduleName")
            for m in mdls:
                vrs = ProductVersion.objects.filter(productmodule=m).order_by("productVersionName")
                for v in vrs:
                    orderedVersions.append(v)
...
class DatabaseVersion(models.Model):
    ...
    def getModelsByTotalOrder():
        orderedVersions = []
        dbs = DatabaseInfo.objects.all().order_by("databaseName")
        for p in dbs:
            mdls = DatabaseVersion.objects.filter(database=p).order_by("databaseVersionName") 
            for v in mdls:
                orderedVersions.append(v)
        return orderedVersions
...
class ServerVersion(models.Model):
    ...
    def getModelsByTotalOrder():
        orderedVersions = []
        svs = ServerInfo.objects.all().order_by("serverName")
        for p in svs:
            mdls = ServerVersion.objects.filter(server=p).order_by("serverVersionName") 
            for v in mdls:
                orderedVersions.append(v)
        return orderedVersions
